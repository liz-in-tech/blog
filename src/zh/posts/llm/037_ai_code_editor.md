...
- 功能：
  ...
  - Add Context 添加上下文
    ...
    - CMD + L
      ...
      - 不选中任何部分，把该文档加入chat
  - .voidrules 文件
    - 类似 Cursor 的 .cursorrules，定义代码风格、框架限制等规则，确保 AI 输出符合项目规范

## 2. 源码整体逻辑
![源码整体逻辑](../../../assets/037_void_sourcecode.png)
...
#### 3.1.1. 主要功能
- 通过判断消息数量和线程ID，动态切换“初始页”和“对话页”（如果当前线程没有任何消息，则为“初始页”）
  - 初始页（landingPageContent， 开始对话之前）
    - landingPageInput（输入框）
      - inputChatArea 输入框区域
        - onSubmit 提交
        - onAbort 取消
    - PastThreadsList（历史对话列表） 或 initiallySuggestedPromptsHTML（建议问题列表）
      - 如果没有“历史对话列表”，则展示“建议问题列表”
  - 对话页（threadPageContent，用户开始对话后：进入完整聊天界面，支持多轮对话、工具调用、代码编辑等）
    - messagesHTML（当前对话的消息列表）
      - previousMessagesHTML
        - 遍历当前线程的所有消息，每条消息用消息气泡 ChatBubble 组件渲染
        - ChatBubble 会根据消息类型选择对应的消息类型组件进行渲染
          - 消息类型
      ...
      - currStreamingMessageHTML
        - 一个消息类型为 assistant 的 ChatBubble
      - generatingTool
        - BuiltInTool 内置工具
        - MCPTool MCP工具
      - loading indicator
...
#### 3.2.1. 主要功能
- addUserMessageAndStreamResponse : 用户在侧边栏输入内容并发送后
  - 在当前线程中添加一条用户消息
  - 自动驱动LLM回复或工具调用主循环
...
#### 3.8.1. 主要功能
- sendLLMMessageToProviderImplementation：实际调用相应的SDK/API
...
- edit_code_tools (AI调用编辑代码工具时)
  ...
  - edit_file：按块查找-替换（只替换指定的代码块，保留未变部分）
    - instantlyApplySearchReplaceBlocks
      - 入参：`{ uri: URI, searchReplaceBlocks: string }`
      - 启动diff区域，勇于显示新内容在旧内容之上哪里进行了变更
